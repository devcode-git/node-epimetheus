...
const metric = {
  http: {
    requests: {
      count: new client.Counter('http_request_count', 'request counter', ['method', 'path', 'cardinality', 'status']),
...
function observe(method, path, statusCode, start) {
  ...
  if (path !== '/metrics' && path !== '/metrics/') {
    ...
    metric.http.requests.count.labels(method, split.path, split.cardinality, statusCode).inc();
